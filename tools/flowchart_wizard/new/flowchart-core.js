(function (root, factory) {
  ...
  const KID_SHAPE_LIBRARY = Object.freeze({
    'start-end': Object.freeze({
      type: 'start-end',
      label: 'Початок / Кінець',
      description: 'Старт або завершення алгоритму.',
      exampleText: 'Початок',
    }),
    'process': Object.freeze({
      type: 'process',
      label: 'Дія',
      description: 'Один простий крок, який треба виконати.',
      exampleText: 'Взяти олівець',
    }),
    'decision': Object.freeze({
      type: 'decision',
      label: 'Умова',
      description: 'Перевірка: так чи ні?',
      exampleText: 'Є зошит?',
    }),
    'input-output': Object.freeze({
      type: 'input-output',
      label: 'Ввід / Вивід',
      description: 'Отримати дані або показати результат.',
      exampleText: 'Ввести ім’я',
    }),
    'subroutine': Object.freeze({
      type: 'subroutine',
      label: 'Підпрограма',
      description: 'Окрема маленька команда, яку можна викликати.',
      exampleText: 'Намалювати будиночок',
    }),
    'connector': Object.freeze({
      type: 'connector',
      label: 'З’єднувач',
      description: 'Мітка для переходу в інше місце схеми.',
      exampleText: 'A',
    }),
  });

  const ROUTE_MODES = ['auto', 'vertical', 'horizontal', 'bypass-left', 'bypass-right'];
  const STRAIGHT_TOLERANCE = 18;
  ...
  const KID_WIZARD_TYPES = Object.freeze([
    'start-end',
    'process',
    'decision',
    'input-output',
  ...
  function getShapeLibrary() {
    return Object.keys(KID_SHAPE_LIBRARY).map((type) => ({
      ...KID_SHAPE_LIBRARY[type],
    }));
  }
  ...
  function getShapeSize(type) {
    const normalized = normalizeShapeType(type);
    if (normalized === 'connector') return { width: 60, height: 60 };
    if (normalized === 'decision') return { width: 144, height: 144 };
    if (normalized === 'start-end') return { width: 164, height: 58 };
    if (normalized === 'input-output') return { width: 180, height: 58 };
    if (normalized === 'subroutine') return { width: 180, height: 58 };
    return { width: 164, height: 58 };
  }

  function isEndTerminator(shape) {
    return normalizeShapeType(shape?.type) === 'start-end'
      && normalizeLabelText(shape?.textRaw) === 'кінець';
  }

  function getQuickAddTargets(shapes, connections) {
    const safeShapes = Array.isArray(shapes) ? shapes : [];
    const safeConnections = Array.isArray(connections) ? connections : [];
    if (!safeShapes.length) {
      return [{ sourceId: null, branch: null, type: 'root' }];
    }

    return safeShapes.flatMap((shape) => {
      const outgoing = safeConnections.filter((conn) => conn.from === shape.id);
  ...
  function getQuickAddPlacement(parentShape, branch, childType) {
    const parentType = normalizeShapeType(parentShape?.type);
    const parentSize = {
      width: Number(parentShape?.width) || getShapeSize(parentType).width,
      height: Number(parentShape?.height) || getShapeSize(parentType).height,
    };
    const childSize = getShapeSize(childType);
    const left = Number(parentShape?.left) || 0;
    const top = Number(parentShape?.top) || 0;
    const branchGapX = 200;
    const branchGapY = 44;
    const linearGapY = 42;
    const childLift = getDecisionVisualLift({ ...childSize, type: childType });

    if (branch === 'yes') {
      return {
        left: Math.round(left - branchGapX),
        top: Math.round(top + parentSize.height + branchGapY + childLift),
      };
    }
    ...
    return {
      left: Math.round(left + (parentSize.width - childSize.width) / 2),
      top: Math.round(top + parentSize.height + linearGapY + childLift),
    };
  }
  ...
  function smartWrapText(raw, type) {
    ...
    const maxLines = (type === 'decision') ? 4 : (type === 'connector') ? 1 : 3;
  ...
  function getDecisionVisualLift(shape) {
    return normalizeShapeType(shape?.type) === 'decision'
      ? Math.max(0, decisionVertexDistance(shape) - ((shape.height || 0) / 2))
      : 0;
  }
  ...
  return {
    ...
    getDecisionVisualLift,
    getDefaultText,
    getEdgePoints,
    getQuickAddPlacement,
    getQuickAddTargets,
    getShapeInfo,
    getShapeLibrary,
    getShapeSize,
