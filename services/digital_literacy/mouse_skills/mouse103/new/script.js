(() => {
    ...
    const MODES = {
        beginner: {
            ...
            baseSpeed: 340, speedInc: 14, speedStep: 6,
            spawnMs: 1400, minSpawnMs: 600, tighten: 4,
    ...
    function toast(msg, dur = 1400) {
        ...
        toast._t = setTimeout(() => UI.toast.classList.add('hidden'), dur);
    }
    ...
    const Metrics = {
        w: 0, h: 0, laneW: 0,
        playerH: 0, playerBottom: 0, playerY: 0,
        measure() {
    ...
    const Game = {
        ...
        bindInputs() {
            ...
            UI.game.addEventListener('touchend',   e => {
                ...
                this.move(Math.abs(dx) > 35 ? (dx < 0 ? -1 : 1) : (e.changedTouches[0].clientX < window.innerWidth / 2 ? -1 : 1));
            }, { passive: true });
        },
        ...
        exitToMenu(withConfirm) {
            if (withConfirm && State.running) {
                if (!window.confirm('Вийти в меню? Заїзд завершиться.')) return;
                this.finish('exit'); return;
            }
        ...
        spawn() {
            ...
            const extra = data.shadow ? 'game-shadow' : 'drop-shadow-md';
            const style = data.transform ? `style="transform:${data.transform}"` : '';
            el.innerHTML = `<i class="fa-solid ${data.icon} fa-3x ${data.color} ${extra}" ${style}></i>`;
        ...
        finish(reason) {
            ...
            UI.finish.rewardTxt.textContent = r.reward;
            ...
            if (newGlobal || newMode) { confettiBurst(true); if (reason !== 'exit') Sound.win(); }
        ...
        hud(force = false) {
            ...
            const modeTxt = State.mode ? State.mode.badge : 'ОБЕРИ РЕЖИМ';
            if (UI.mode.textContent !== modeTxt) UI.mode.textContent = modeTxt;

            upd(UI.score, String(State.score));
            upd(UI.high,  String(State.globalHigh));
            ...
            if (!State.mode) {
                ...
                lHtml = '<i class="fa-solid fa-infinity text-slate-500 text-sm"></i>';
            } else {
                const max = State.s.lives;
                lHtml = '';
                for (let i = 0; i < max; i++) {
                    lHtml += `<i class="fa-solid fa-heart ${i < State.lives ? 'heart' : 'empty'} text-sm"></i>`;
                }
            }
            if (UI.lives.innerHTML !== lHtml) { UI.lives.innerHTML = lHtml; faSync(UI.lives); }
        },
