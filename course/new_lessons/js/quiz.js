...
import { escapeHtml, completeTask, setStatus, updateProgress } from "./shared.js";
          ...
          <div class="option-list">
            ${question.options.map((option, optionIndex) => `
              <label for="${question.id}-${optionIndex}">
                <input id="${question.id}-${optionIndex}" type="${type}" name="${question.id}" value="${escapeHtml(option)}" ${savedValues.includes(option) ? "checked" : ""}>
...
export function setupQuiz(quiz, state, refs, showFeedback) {
  ...
  refs.checkQuizHandler = () => {
    ...
    state.completed.quiz = ok;
    persistState(state);
    updateProgress(state, refs);
    setStatus(refs.quizResult, `Результат: ${score} із ${quiz.length}.`, ok ? "is-success" : score > 0 ? "is-warning" : "is-danger");
    showFeedback(ok ? "Тест виконано без помилок." : `Тест: ${score} з ${quiz.length}.`, ok ? "is-success" : "is-warning", ok ? "✓" : "!");
  };
