    ...
    <article class="task-card" data-activity-id="${activity.id}">
      ...
      <div class="canvas-fallback">
        ...
        <select id="draw-fallback">
          ...
          ${activity.fallbackOptions.map((option) => `<option value="${escapeHtml(option)}" ${selectedFallback === option ? "selected" : ""}>${escapeHtml(option)}</option>`).join("")}
        </select>
      </div>
...
export function setupDrawTask(activity, state, refs, showFeedback) {
  ...
  if (!canvas || !fallbackSelect) return;

  const context = canvas.getContext("2d");
  const rootStyles = getComputedStyle(document.documentElement);
  const strokeColor = rootStyles.getPropertyValue("--primary").trim() || rootStyles.getPropertyValue("--color-primary").trim() || "black";
  let drawing = false;
  ...
  document.getElementById("clear-canvas").addEventListener("click", () => {
    ...
    persistState(state);
    setStatus(refs.activities.querySelector(`[data-feedback="${activity.id}"]`), "Малюнок очищено. Можна почати ще раз.", "is-warning");
    showFeedback("Малюнок очищено.", "is-warning", "!");
  });

  document.getElementById("mark-draw-done").addEventListener("click", () => {
    completeTask("draw", state, refs);
    setStatus(refs.activities.querySelector(`[data-feedback="${activity.id}"]`), "Завдання з малювання позначено як виконане.", "is-success");
