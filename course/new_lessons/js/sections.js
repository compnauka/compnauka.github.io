  ...
  container.innerHTML = sections.map((section) => {
    const cardsMarkup = section.cards
      ? `<div class="info-grid">${section.cards.map((card) => {
        const cardAudienceClass = resolveAudienceClass(card.audience);
        return `<article class="info-box ${cardAudienceClass}"><h3>${escapeHtml(card.title)}</h3><p>${escapeHtml(card.text)}</p></article>`;
      }).join("")}</div>`
      : "";
