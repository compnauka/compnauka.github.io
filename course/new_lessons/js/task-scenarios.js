...
export function renderScenariosTask(activity, state) {
  ...
  const completedCount = activity.situations.filter((_, index) => state.activityState[`${activity.id}-checked-${index}`]).length;
...
export function setupScenariosTask(activity, state, refs, showFeedback, rerenderActivities) {
  ...
  document.querySelectorAll(`[data-check-scenario="${activity.id}"]`).forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (ok) {
        const allDone = activity.situations.every((_, index) => state.activityState[`${activity.id}-checked-${index}`]);
      ...
      setStatus(feedback, ok ? "Правильно! У цій ситуації обрано всі потрібні варіанти." : "Є помилки. Спробуй подумати ще раз.", ok ? "is-success" : "is-warning");
      showFeedback(ok ? "Ситуацію розв’язано правильно." : "У виборі є помилки.", ok ? "is-success" : "is-warning", ok ? "✓" : "!");
    });
  });
